...
def weather_api_call(api_key, location_id):
    ...
    return [str(int(w.get_temperature(unit='celsius')['temp'])), w.get_weather_icon_name()]
...
def draw_temperature(temp):
    ...
    elif int(temp) < 0 and len(temp) == 3:

        draw_digits.draw_digit(temp[1:2], 11, 5, 255, 0, 0)
        draw_digits.draw_digit(temp[2:], 7, 5, 255, 0, 0)
